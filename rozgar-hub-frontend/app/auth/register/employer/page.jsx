...
export default function EmployerRegisterPage() {
  ...
  async function handleDocsSubmit(e) {
    ...
    try {
      ...
      const res = await fetch("http://localhost:5000/api/auth/register/employer", {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#1e3a8a] via-[#172554] to-[#0f172a] px-4 py-10">
      <div className="max-w-lg w-full bg-white/95 backdrop-blur-md p-8 rounded-2xl shadow-2xl space-y-6 border border-white/20">
        ...
        <h1 className="text-2xl font-bold text-center text-[#1e3a8a]">Employer Registration</h1>

        {/* ── STEP 1: Basic Info ── */}
        {step === "form" && (
          <form onSubmit={handleFormNext} className="space-y-4">
            <Input placeholder="Company / Full Name" value={form.name} error={errors.name}
            ...
            <Input placeholder="Phone (03001234567)" value={form.phone}
